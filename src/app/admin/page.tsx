...
import React, { useState, useEffect, FC, PropsWithChildren, ComponentProps } from 'react';
import { AnimatePresence, motion } from 'framer-motion';
import { Settings, Menu, X, FileText, PlusCircle, Trash2, Edit, MessageSquare, Download, Calendar, Search } from 'lucide-react';
        ...
        <div className="p-4 md:p-8 w-full h-full flex flex-col">
            ...
            <div className="bg-neutral-200 dark:bg-neutral-800 p-4 rounded-xl flex-1 flex flex-col shadow-lg">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 flex-1">
                    <textarea value={markdownContent} onChange={(e) => setMarkdownContent(e.target.value)} className="w-full h-full p-4 rounded-lg bg-white dark:bg-black/40 border border-gray-300 dark:border-gray-700 resize-none font-mono focus:ring-2 focus:ring-blue-500" />
                ...
                <div className="mt-4 flex items-center gap-4 pt-4 border-t border-neutral-300 dark:border-neutral-700">
                    <input type="email" value={authorEmail} onChange={(e) => setAuthorEmail(e.target.value)} placeholder="作者邮箱" className="px-3 py-2 rounded-lg bg-white dark:bg-black/40 border" />
                    <button onClick={handleSubmit} disabled={isSubmitting} className="px-6 py-2 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 disabled:bg-gray-500">{isSubmitting ? '提交中...' : isEditMode ? '更新文章' : '发布文章'}</button>
...
const ArticleList: FC<{ articles: Article[]; isLoading: boolean; error: string | null; onEdit: (article: Article) => void; onDelete: (articleId: string) => void; }> = ({ articles, isLoading, error, onEdit, onDelete }) => {
    ...
    useEffect(() => {
        ...
        if (keyword) {
            tempArticles = tempArticles.filter(article => article.title.toLowerCase().includes(keyword.toLowerCase()));
        }
        if (author) {
            tempArticles = tempArticles.filter(article => article.authorEmail.toLowerCase().includes(author.toLowerCase()));
        }
        if (startDate) {
            tempArticles = tempArticles.filter(article => new Date(article.createdAt) >= new Date(startDate));
        }
        if (endDate) {
            const endOfDay = new Date(endDate);
            endOfDay.setHours(23, 59, 59, 999);
            tempArticles = tempArticles.filter(article => new Date(article.createdAt) <= endOfDay);
        }
    ...
    const handleExport = () => {
        ...
        const dataToExport = filteredArticles.map(article => ({
            ...
            '发布日期': new Date(article.createdAt).toLocaleDateString(),
            '封面图URL': article.coverImageUrl,
        }));
        ...
        <div className="p-4 md:p-8 w-full h-full flex flex-col">
            ...
            <div className="flex flex-wrap items-center gap-4 mb-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                <div className="relative">
                    ...
                    <input type="text" placeholder="按标题筛选..." value={keyword} onChange={e => setKeyword(e.target.value)} className="p-2 pl-10 rounded border bg-white dark:bg-gray-700"/>
                </div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="p-2 rounded border bg-white dark:bg-gray-700"/>
                    <span>-</span>
                    <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="p-2 rounded border bg-white dark:bg-gray-700"/>
                </div>
                    ...
                    <tbody>
                        {isLoading ? (<tr><td colSpan={5} className="text-center p-8">正在加载...</td></tr>) : 
                         ...
                         (filteredArticles.map((article) => (
                            <tr key={article.id} className="bg-white border-b dark:bg-gray-800 hover:bg-gray-600">
                                ...
                                <th scope="row" className="px-6 py-4 font-medium text-gray-900 dark:text-white">{article.title}</th><td className="px-6 py-4">{article.authorEmail}</td><td className="px-6 py-4">{new Date(article.createdAt).toLocaleDateString()}</td>
                                <td className="px-6 py-4 text-center"><button onClick={() => onEdit(article)} className="font-medium text-blue-500 hover:underline mr-4"><Edit className="inline h-5 w-5"/></button><button onClick={() => onDelete(article.id)} className="font-medium text-red-500 hover:underline"><Trash2 className="inline h-5 w-5"/></button></td>
                            </tr>
                        )))}
                    </tbody>
                </table>
...
const ChatLogViewer: FC<{ logs: ChatLog[]; isLoading: boolean; error: string | null; onRefresh: () => void; }> = ({ logs, isLoading, error, onRefresh }) => {
    ...
    useEffect(() => {
        let tempLogs = [...logs];
        if (startDate) {
            tempLogs = tempLogs.filter(log => new Date(log.timestamp) >= new Date(startDate));
        }
        if (endDate) {
            const endOfDay = new Date(endDate);
            endOfDay.setHours(23, 59, 59, 999);
            tempLogs = tempLogs.filter(log => new Date(log.timestamp) <= endOfDay);
        }
        setFilteredLogs(tempLogs.sort((a,b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()));
    ...
    const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.checked) {
            setSelectedKeys(new Set(filteredLogs.map(log => log.key)));
        } else {
    ...
    const handleDelete = async (keysToDelete: string[]) => {
        ...
        if (!window.confirm(`确定要删除选中的 ${keysToDelete.length} 条记录吗？`)) return;
    ...
    const handleExport = () => {
        ...
        const dataToExport = filteredLogs.map(log => ({
            '用户邮箱': log.user.email,
            '用户问题': log.question,
            '提问时间': new Date(log.timestamp).toLocaleString(),
        }));
        const worksheet = XLSX.utils.json_to_sheet(dataToExport);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "客户问题记录");
        XLSX.writeFile(workbook, "客户问题记录.xlsx");
    };
        ...
        <div className="p-4 md:p-8 w-full h-full flex flex-col">
            ...
            <div className="flex-1 overflow-x-auto shadow-md sm:rounded-lg">
                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            <th scope="col" className="p-4"><input type="checkbox" onChange={handleSelectAll} checked={filteredLogs.length > 0 && selectedKeys.size === filteredLogs.length}/></th>
                            <th scope="col" className="px-6 py-3">用户</th>
                            <th scope="col" className="px-6 py-3">问题内容</th>
                    ...
                    <tbody>
                        {isLoading ? (<tr><td colSpan={5} className="text-center p-8">正在加载...</td></tr>) : 
                         error ? (<tr><td colSpan={5} className="text-center p-8 text-red-500">{error}</td></tr>) : 
                         filteredLogs.length === 0 ? (<tr><td colSpan={5} className="text-center p-8">没有符合条件的记录</td></tr>) : 
                         (filteredLogs.map((log) => (
                            <tr key={log.key} className="bg-white border-b dark:bg-gray-800 hover:bg-gray-600">
                                <td className="p-4"><input type="checkbox" checked={selectedKeys.has(log.key)} onChange={() => handleSelect(log.key)}/></td>
                                <td className="px-6 py-4">{log.user.email}</td>
                                <td className="px-6 py-4 max-w-md truncate" title={log.question}>{log.question}</td>
                                <td className="px-6 py-4">{new Date(log.timestamp).toLocaleString()}</td>
                                <td className="px-6 py-4"><button onClick={() => handleDelete([log.key])} className="text-red-500 hover:underline"><Trash2 className="h-5 w-5"/></button></td>
                            </tr>
                        )))}
                    </tbody>
                </table>
...
export default function AdminPage() {
    ...
    const renderView = () => {
        switch(view) {
            case 'list':
                return <ArticleList articles={articles} isLoading={isArticlesLoading} error={articlesError} onEdit={handleEditArticle} onDelete={handleDeleteArticle} />;
            case 'editor':
                return <ArticleEditor onArticlePublished={handlePublishSuccess} articleToEdit={editingArticle} />;
            case 'questions':
                return <ChatLogViewer logs={chatLogs} isLoading={isChatLogsLoading} error={chatLogsError} onRefresh={fetchChatLogs} />;
            default:
                return <div>请选择一个视图</div>;
        }
    };
