...
function CurrencyConverter() {
    ...
    const fetchRates = useCallback(async () => {
        ...
        try {
            ...
            if (!response.ok) throw new Error('网络响应失败，无法获取最新汇率。');
            const data = await response.json();
            if (data.result === 'error') throw new Error(data['error-type'] || '获取汇率数据时发生未知错误。');
            setRates(data.rates);
    ...
    const calculateConvertedAmount = useCallback((val1: number | string, c1: Currency, c2: Currency, setOtherAmount: (v: string) => void) => {
        if (Object.keys(rates).length === 0 || isLoading) return;
        const rate1 = rates[c1];
        const rate2 = rates[c2];
        const numValue = typeof val1 === 'string' ? parseFloat(val1) : val1;
        if (rate1 && rate2 && !isNaN(numValue)) {
            const result = (numValue / rate1) * rate2;
            setOtherAmount(result.toFixed(4));
        } else {
            setOtherAmount('');
        }
    }, [rates, isLoading]);
    ...
    useEffect(() => {
        calculateConvertedAmount(amount1, currency1, currency2, (v) => setAmount2(v));
    }, [amount1, currency1, currency2, calculateConvertedAmount]);
    ...
    const handleAmount2Change = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setAmount2(value);
        calculateConvertedAmount(value, currency2, currency1, (v) => setAmount1(v));
    };
            ...
            {!isLoading && !error && (
                 <div className="space-y-4">
                    ...
                    <div className="flex justify-center pt-4">
                        <button 
                            onClick={() => router.push('/')}
                            className="bg-gray-600/50 hover:bg-gray-500/50 text-white py-2 px-6 rounded-lg transition-colors duration-300"
                        >
                            返回主页
                        </button>
                    </div>
...
function ExchangePage({ title = "Apex" }: { title?: string }) {
...
export default function Page() {
    return <ExchangePage title="Apex Exchange" />;
}
