...
const SelectField: FC<{ label: string; name: string; options: Option[]; value: string; onChange: (e: ChangeEvent<HTMLSelectElement>) => void }> = ({ label, name, options, value, onChange }) => ( <div className="mb-4"> <label className="block text-gray-700 text-sm font-bold mb-2 text-left">{label}</label> <select name={name} value={value} onChange={onChange} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"> {options.map(option => ( <option key={option.value} value={option.value}>{option.label}</option> ))} </select> </div> );
const FileUploadField: FC<{ label: string; onFileChange: (files: File[]) => void; files: File[]; fileError?: string }> = ({ label, onFileChange, files = [], fileError }) => { const handleFileChange = (event: ChangeEvent<HTMLInputElement>) => { const selectedFiles = event.target.files; if (selectedFiles && selectedFiles.length > 0) { const fileList = Array.from(selectedFiles); onFileChange(fileList); } }; const handleRemoveFile = (fileNameToRemove: string) => { const updatedFiles = files.filter(file => file.name !== fileNameToRemove); onFileChange(updatedFiles); }; return ( <div className="mb-4"> <label className="block text-gray-700 text-sm font-bold mb-2 text-left">{label}</label> <div className="relative border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-500 transition-colors"> <div className="flex flex-col items-center"> <svg className="w-12 h-12 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" /> </svg> <p className="mt-2 text-sm text-gray-600"> <span className="font-semibold text-blue-600">点击上传</span> 或拖拽文件到此区域 </p> <p className="text-xs text-gray-500 mt-1">支持图片、PDF、压缩包等，单个文件不超过 10MB</p> {fileError && <p className="text-sm text-red-600 mt-2 font-semibold">{fileError}</p>} </div> <input type="file" multiple accept="image/*,application/pdf,.zip,.rar,.7z" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" onChange={handleFileChange} /> </div> {files.length > 0 && ( <div className="mt-4"> <h4 className="text-sm font-semibold text-gray-700 mb-2">已选择的文件:</h4> <ul className="list-disc pl-5 space-y-1"> {files.map((file, index) => ( <li key={index} className="text-sm text-gray-600 flex justify-between items-center"> <span>{file.name} ({(file.size / 1024 / 1024).toFixed(2)} MB)</span> <button type="button" onClick={() => handleRemoveFile(file.name)} className="text-red-500 hover:text-red-700 text-xs font-bold">移除</button> </li> ))} </ul> </div> )} </div> ); };
const RadioGroupField: FC<{ label: string, name: string, options: Option[], value: string, onChange: (e: { target: {name: string, value: string} }) => void }> = ({ label, name, options, value, onChange }) => ( <div className="mb-4"> <label className="block text-gray-700 text-sm font-bold mb-2 text-left">{label}</label> <div className="flex items-center space-x-4 flex-wrap"> {options.map(option => ( <label key={option.value} className="flex items-center mr-4 mb-2 cursor-pointer text-gray-800"> <input type="checkbox" name={name} value={option.value} checked={value === option.value} onChange={(e) => { const newValue = e.target.value; const finalValue = value === newValue ? '' : newValue; onChange({ target: { name, value: finalValue }}); }} className="mr-2 h-4 w-4 rounded text-blue-600 border-gray-300 focus:ring-blue-500" /> {option.label} </label> ))} </div> </div> );
const CheckboxGroupField: FC<{ label: string, value: string[], onChange: (e: ChangeEvent<HTMLInputElement>) => void, options: Option[] }> = ({ label, value = [], onChange, options }) => ( <div className="mb-4"> <label className="block text-gray-700 text-sm font-bold mb-2 text-left">{label}</label> <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3"> {options.map(option => ( <label key={option.value} className="flex items-center whitespace-nowrap cursor-pointer text-gray-800"> <input type="checkbox" value={option.value} checked={value.includes(option.value)} onChange={onChange} className="mr-2 h-4 w-4 rounded text-blue-600 border-gray-300 focus:ring-blue-500" /> {option.label} </label> ))} </div> </div> );
const TextareaField: FC<{ label: string, placeholder?: string, value: string, onChange: (e: ChangeEvent<HTMLTextAreaElement>) => void }> = ({ label, placeholder, value, onChange }) => ( <div className="mb-4"> <label className="block text-gray-700 text-sm font-bold mb-2 text-left">{label}</label> <textarea className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" rows={4} placeholder={placeholder} value={value} onChange={onChange}></textarea> </div> );
...
const UploadModal: FC<UploadModalProps> = ({ isOpen, onClose, selectedServiceIds, formData, setFormData, submissionStatus, setSubmissionStatus, currentUser }) => {
    ...
    const renderField = (field: Field) => {
        ...
        switch (Component) {
            case SectionHeader: case SubHeader: return <Component key={id} {...props} />;
            case FormField: case TextareaField: case SelectField: return <Component key={id} {...props} value={(value as string) || ''} onChange={(e: ChangeEvent<HTMLInputElement|HTMLTextAreaElement|HTMLSelectElement>) => handleFormChange(id, e.target.value)} />;
            case RadioGroupField: return <Component key={id} {...props} value={(value as string) || ''} onChange={(e: { target: {name:string;value:string;} }) => handleFormChange(id, e.target.value)} />;
            case CheckboxGroupField: return <Component key={id} {...props as {label:string,options:Option[]}} value={(value as string[]) || []} onChange={(e:ChangeEvent<HTMLInputElement>)=>{const{value:v,checked}=e.target;const c=(formData[id] as string[])||[];const n=checked?[...c,v]:c.filter(i=>i!==v);handleFormChange(id,n)}} />;
            case FileUploadField: return <Component key={id} {...props as { label: string }} onFileChange={(files: File[]) => handleFileChange(id, files)} files={(value as FileData)?.files || []} fileError={(value as FileData)?.error} />;
            case TableField: return <Component key={id} {...props as {label:string,columns:Column[]}} value={(value as TableRow[])||[]} onChange={(newValue:TableRow[])=>handleFormChange(id,newValue)} />;
            case DynamicPersonField: return <Component key={id} {...props as {title?:string,personType:string,fieldSet:Field[],max?:number}} value={(value as PersonData[])||[]} onChange={(newValue:PersonData[])=>handleFormChange(id,newValue)} />;
            default: return null;
        }
    };
        ...
        <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
            <motion.div initial={{y:-50,opacity:0}} animate={{y:0,opacity:1}} exit={{y:-50,opacity:0}} className="bg-white rounded-lg shadow-xl p-4 sm:p-6 md:p-8 w-11/12 sm:w-5/6 md:w-4/5 lg:w-3/4 xl:max-w-4xl max-h-[90vh] overflow-y-auto" onClick={(e)=>e.stopPropagation()}>
                ...
                <div className="mt-8 flex justify-end">
                    ...
                    <button onClick={handleSubmit} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline disabled:bg-blue-400 disabled:cursor-wait" disabled={submissionStatus==='loading' || !currentUser}>
...
export default function UploadPage() {
    ...
    if (!user) {
        return (
            <div className="min-h-screen w-full flex items-center justify-center bg-black">
                <p className="text-white">加载中或验证用户身份...</p>
            </div>
        );
    }
    ...
    return (
        ...
        >
            ...
            <div className="relative z-10 container mx-auto px-2 md:px-6 flex flex-col items-center w-full">
                <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{duration:2}} className="w-full max-w-4xl mx-auto text-center">
                    <h1 className="text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-bold mb-12 tracking-tighter">
                     ...
                     <p className="text-gray-300 text-lg">欢迎, {user.name} ({user.email})</p>
                </motion.div>
                <motion.div className="w-full max-w-4xl" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.8,delay:0.5}}>
                    ...
                    <div className="flex justify-center items-center gap-4">
                        <button onClick={()=>{setFormData({});setModalOpen(true);}} disabled={selectedServices.length===0} className="bg-green-600 text-white font-bold py-3 px-10 rounded-lg shadow-lg transition-all duration-300 transform hover:scale-105 disabled:bg-gray-400 disabled:cursor-not-allowed disabled:scale-100">
                         ...
                         <button onClick={() => router.push('/')} className="inline-block bg-gray-200/20 hover:bg-gray-200/30 backdrop-blur-sm text-gray-200 font-bold py-3 px-10 rounded-lg shadow-lg transition-all duration-300 transform hover:scale-105">
                            返回主页
                        </button>
                    </div>
                </motion.div>
            ...
            <AnimatePresence>
                {isModalOpen && (
                    <UploadModal isOpen={isModalOpen} onClose={()=>setModalOpen(false)} selectedServiceIds={selectedServices} formData={formData} setFormData={setFormData} submissionStatus={submissionStatus} setSubmissionStatus={setSubmissionStatus} currentUser={user} />
                )}
            </AnimatePresence>
            <AnimatePresence>
